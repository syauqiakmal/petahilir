...
import { PopupComponentshapfile } from "../layers/legend_shapfile";
...
export const Map = ({ hideComponents }) => {
  ...
  const [selectedProperty, setSelectedProperty] = useState("ibt_stat");
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const response = await fetch("https://petahilirisasi.id/map/shapefile");
        ...
        setGeojsonData(geojsonData); // Set fetched GeoJSON as default
