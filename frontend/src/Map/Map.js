...
export const Map = ({ hideComponents }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      // Upload file
      const uploadResponse = await fetch("http://backend:8000/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (file.name.endsWith(".zip")) {
        const dataResponse = await fetch(
          `http://backend:8000/data/${tableName}`
        );
        if (!dataResponse.ok) {
      ...
      // Handle raster file
        ...
        const dataResponse = await fetch(
          `http://backend:8000/raster/${tableName}`
        );
        if (!dataResponse.ok) {
      ...
      // Handle geojson file
        ...
        const dataResponse = await fetch(
          `http://backend:8000/geojson/${tableName}`
        );
        if (!dataResponse.ok) {
          throw new Error(
  ...
  useEffect(() => {
    const fetchRaster = async () => {
      try {
        const response = await fetch(
          `http://backend:8000/rasterNikelA/`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch raster data");
        }
  ...
  useEffect(() => {
    const fetchRaster = async () => {
      try {
        const response = await fetch(
          `http://backend:8000/rasterNikelB/`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch raster data");
        }
  ...
  useEffect(() => {
    const fetchRaster = async () => {
      try {
        const response = await fetch(
          `http://backend:8000/rasterNikelC/`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch raster data");
        }
  ...
  useEffect(() => {
    const fetchRaster = async () => {
      try {
        const response = await fetch(
          `http://backend:8000/rasterNikelD/`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch raster data");
        }
  ...
  useEffect(() => {
    const fetchRaster = async () => {
      try {
        const response = await fetch(
          `http://backend:8000/rasterNikelE/`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch raster data");
        }
